...
def log_to_wandb(embeds, markup, dist_threshold, url2record, text_to_vec_func):
    clustering_model = AgglomerativeClustering(
        n_clusters=None,
        distance_threshold=dist_threshold,
        linkage="single",
    ...
    dict_res = calc_clustering_metrics(markup, url2label, url2record, output_dict=True)

    wandb.run.summary.update({
        f'{text_to_vec_func} best distance': dist_threshold,
        'Clustering results': dict_res,
    })
...
if __name__ == "__main__":
    ...
    parser.add_argument("--text-to-vec-func", type=str, required=True, choices=('bert-MeanSum', 'bert-FirstCLS'))
